...
//The EventEmitter class is used to create objects that can emit named
//events and handle them asynchronously.
...
class TextToSpeechService2 extends EventEmitter {
  constructor(config) {
    ...
    // then assign this.config.voiceId to process.env.VOICE_ID_RICHARD
    this.config.voiceId ||= process.env.VOICE_ID_RICHARD;
    ...
    try {
      ...
      const response = await fetch(
        ...
        `https://api.elevenlabs.io/v1/text-to-speech/${this.config.voiceId}/stream?output_format=${outputFormat}&optimize_streaming_latency=3`,
        {
          ...
          //Converts the object to a JSON string to be sent as the body of the POST request.
          body: JSON.stringify({
...
//By exporting TextToSpeechService2, you allow other files to import and use this class.
